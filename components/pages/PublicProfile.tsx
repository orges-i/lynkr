...
const PublicProfile: React.FC = () => {
    ...
    if (error || !profile || !appearance) {
        return (
            <div className="min-h-screen bg-zinc-950 flex flex-col items-center justify-center p-6 text-center text-white">
                ...
                <p className="text-zinc-400 text-lg mb-8">{error || 'Page not found'}</p>
                <RouterLink to="/" className="bg-white text-black px-8 py-3 rounded-full font-bold hover:scale-105 transition-transform">
                    Back to LYNKR
                </RouterLink>
            </div>
        );
    ...
    if (appearance.font_color && appearance.font_color !== 'auto') {
        ...
        // Auto logic
        const darkThemes = ['dark', 'midnight', 'sunset', 'ocean', 'forest', 'custom'];
    ...
    const customBackgroundStyle = appearance.theme === 'custom' && appearance.custom_theme_url
    ...
    const getSocialIcon = (url: string) => {
        ...
        if (lower.includes('twitter.com') || lower.includes('x.com')) return <Twitter className="w-5 h-5" />;
    ...
    return (
        ...
        >
            ...
            {/* Content Wrapper */}
            <div className={`w-full max-w-2xl px-6 flex flex-col items-center ${profile.cover_image_url ? '-mt-16 sm:-mt-20' : 'pt-20 sm:pt-32'} pb-24 relative z-10`}>

                {/* Avatar */}
                <div className={`w-32 h-32 sm:w-40 sm:h-40 rounded-full border-4 border-white shadow-xl overflow-hidden mb-6 bg-white shrink-0`}>
                    <img
                ...
                {/* Profile Info */}
                <h1 className={`text-2xl sm:text-3xl font-bold mb-2 text-center ${textColorClass}`}>
                    @{profile.username}
                </h1>
                {profile.bio && (
                    <p className={`text-center text-lg sm:text-xl opacity-90 mb-10 max-w-lg leading-relaxed ${textColorClass}`}>
                        {profile.bio}
                    </p>
                )}
                ...
                {appearance.show_brand_tag && (
                    <div className="mt-16 sm:mt-24 opacity-60 hover:opacity-100 transition-opacity">
                        <RouterLink to="/" className="flex items-center gap-2">
                            <span className="font-bold tracking-[0.3em] text-xs sm:text-sm uppercase">LYNKR</span>
                        </RouterLink>
                    </div>
                )}
            </div>
